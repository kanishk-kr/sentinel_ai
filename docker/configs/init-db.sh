...
# Creates schema-separated logical stores
# ================================================================

# Read the application role password from the injected Docker secret
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Audit schema: application role can INSERT and SELECT only
